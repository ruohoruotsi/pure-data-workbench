...
static t_class *lpcsyn_tilde_class;

typedef struct _lpcsyn_control
{
...
typedef struct _lpcsyn_tilde
{
  t_object x_obj;
  t_float f_dummy;
  t_int x_order;
  t_lpcsyn_control x_ctl;
} t_lpcsyn_tilde;

t_int *lpcsyn_tilde_perform(t_int *w)
{
  // t_object x_obj;
  t_lpcsyn_tilde     *x =     (t_lpcsyn_tilde *)(w[1]);
  t_lpcsyn_control *ctl =   (t_lpcsyn_control *)(w[2]);
  int                  n =                   (int)(w[3]);
...
static void lpcsyn_tilde_list(t_lpcsyn_tilde *x, t_symbol *s, int argc, t_atom *argv)
{
  if (argc) 
  {
...
void *lpcsyn_tilde_dsp(t_lpcsyn_tilde *x, t_signal **sp)
{
  x->x_ctl.c_residual = sp[0]->s_vec;
  x->x_ctl.c_output = sp[1]->s_vec;
  dsp_add(lpcsyn_tilde_perform, 3, x, &x->x_ctl, sp[0]->s_n);
  return (void *)x;
}

void *lpcsyn_tilde_new(t_floatarg f)
{
  t_lpcsyn_tilde *x = (t_lpcsyn_tilde *)pd_new(lpcsyn_tilde_class);
...
void lpcsyn_tilde_setup(void)
{
  lpcsyn_tilde_class = class_new(gensym("lpcsyn~"), (t_newmethod)lpcsyn_tilde_new, 0, sizeof(t_lpcsyn_tilde), CLASS_DEFAULT, A_DEFFLOAT, 0);

  post(". . Lattice IIR filter for lpc. . . . . . .");
  post(". . by Nicolas Chetry <okin@altern.org> . .");
  post(". & Edward Kelly <morph_2016@yahoo.co.uk> .");
  
  class_addmethod(lpcsyn_tilde_class, (t_method)lpcsyn_tilde_dsp, gensym("dsp"), 0);
//  class_sethelpsymbol(lpcsyn_tilde_class, gensym("help-lpc~"));
  class_addlist(lpcsyn_tilde_class, lpcsyn_tilde_list);
  class_sethelpsymbol(lpcsyn_tilde_class, gensym("lpc-cross-synthesis.pd"));
  CLASS_MAINSIGNALIN(lpcsyn_tilde_class, t_lpcsyn_tilde, f_dummy);
}
